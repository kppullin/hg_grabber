	...
	async.forEachLimit(repoList, parallelThreads, function(repoNode, callback) {
		...
		if (!exists) {
			...
			var child = childProcess.execFile('hg', 
				...
				{ maxBuffers: 1024 * 1024 * 5 }, function(error){
					console.log(error);
				}
			);
...
function outputFailureMsg (operation, repoName, stdoutBuf, stderrBuf) {
...
function getListAndGrab () {

	var auth = 'Basic ' + new Buffer(username + ':' + password).toString('base64'),
		headers = { Authorization: auth };

	console.log('Fetching list of files...');
	var req = https.request({ hostname: url.parse(repoRoot).hostname, rejectUnauthorized: false, headers: headers }, function(res) {
